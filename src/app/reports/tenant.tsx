...
import type { TenantReportData } from "@/types/report";
import { buildTenantReportData } from "@/utils/reports/data";
import {
  formatReportDate,
  formatShortDate,
  sanitizeFileName,
} from "@/utils/reports/format";
import { buildTenantProfileHtml } from "@/utils/reports/html";
...
export default function TenantReportScreen() {
  ...
  const reportData: TenantReportData | null = useMemo(() => {
    if (!resident) return null;
    return buildTenantReportData(resident, hostelData?.hostel, {
      generatedBy,
    });
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      const html = buildTenantProfileHtml(reportData);
      const fileName = sanitizeFileName(`vaas-tenant-${reportData.name}`);
      await downloadReportPdf(html, fileName);
    } catch {
      Alert.alert("Download failed", "Could not generate the tenant PDF.");
    } finally {
  ...
  if (!tenancyId || !hostelId) {
    return (
      <ReportScaffold
        title="Tenant Profile Report"
        loading={false}
        downloadDisabled
        onDownload={() => undefined}
      >
        <Text style={previewStyles.emptyText}>
          Missing tenant information. Open this report from the residents list.
        </Text>
      </ReportScaffold>
    );
  }
  ...
  return (
    ...
    >
      {!loading && !reportData ? (
        <Text style={previewStyles.emptyText}>Tenant not found.</Text>
      ) : null}
